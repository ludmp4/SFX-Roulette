...
If GitHub does not allow archive downloads, for example because the repository is private, the installer falls back to the bundled files in the extracted ZIP and prints that fallback clearly with the bundled package version from `VERSION.json`. For friend testing without GitHub login, make the repo public or send a freshly rebuilt ZIP.
